...
private:
    ...
    atomic<bool> early_termination{false};

    vector<Task> taskData;

public:
    Worker(string testcase_config_path) {
        data_socket = start_connection(8080);
        control_socket = start_connection(8081);
    ...
    void processTasksLoop() {
        while (true) {
            ...
            // if test case is not executed, then verdict is "skipped" (either because of wrong or early termination)
        }
    }
    ...
    Output execute_code(Testcase testcase, int time_limit, int memory_limit, string executable_path) {
        ...
        if (pid == 0) {
            ...
            // Set limits
            struct rlimit rl;
            rl.rlim_cur = rl.rlim_max = (time_limit / 1000) + 2;  // CPU seconds + buffer
            ...
            struct rusage usage;
            int status;
            long timeout_ms = time_limit * 3;  // 3x buffer for wall-clock timeout
            
            struct timespec start_time, current_time;
            clock_gettime(CLOCK_MONOTONIC, &start_time);  // Monotonic = not affected by system time changes
